/*******************************************************************************
   ...
   21/7/2019 Adapted for Apeldoorn-In-Data Hittestress project by Alex Nijmeijer
...
static const uint8_t APPEUI[8] = { 0x1F, 0xFE, 0x01, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 }; // hittestress

// This should also be in little endian format, see above (LSB..MSB)
//static const uint8_t DEVEUI[8] = { 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }; // device ID from excel
static const uint8_t DEVEUI[8] = { 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }; // device ID from excel

// This should also be in big endian format (MSB..LSB) // to be filled in on a per-node basis
//static const uint8_t APPKEY[16] = { 0x11, 0xAE, 0x3C, 0xF8, 0x9F, 0x60, 0xA3, 0xE3, 0xC3, 0x09, 0xDE, 0xCC, 0xE8, 0xF2, 0x51, 0xAE }; // AID Hittestress 2019, generated by TTN console (see also excel)
static const uint8_t APPKEY[16] = { 0xE7, 0x85, 0x41, 0x75, 0xDA, 0x99, 0x0F, 0xB8, 0xCE, 0x00, 0x37, 0x6C, 0x75, 0xA3, 0x23, 0x59 };
...
void loop() 
{
  ...
  Serial.println(F("Before update"));
  eeprom_getMagic(buf);
